...
export function sphere_with_waves() {
  ...
  scene.background = new threeColor(0xa8def0);
  ...
  controls.target = new threeVector3(0, 0, -40);
  controls.update();

  // AMBIENT LIGHT
  scene.add(new threeAmbientLight(0xffffff, 0.5));
  // DIRECTIONAL LIGHT
  const dirLight = new threeDirectionalLight(0xffffff, 1.0);
  ...
  // PLANE
  const geometry = new threeSphereBufferGeometry(6, 128, 128);
  const sphere = new threeMesh(
    geometry,
