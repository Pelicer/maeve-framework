...
class LandingUseCase():
    ...
    def __init__(self, models, force_model_creation):
        ...
        self.evaluation_service = EvaluationService('./datasets/dataset_landing.csv')    
        self.models = models         

    def run(self):
        self.evaluation_service.loadAndPreProcess('contacted')
        self.evaluation_service.plot_distribution('contacted', './datasets/label_distribution/dataset_landing_')

        for model in self.models:
            print(f"Running evaluation for {model}")
            resources_output = f"./models/{model}/output_resources/landing_use_case/"
            ...
            if model == 'KNN':
                for i in n_neighbors_list:
                    ...
                    self.evaluation_service.saveModel(f"./models/{model}/model/landing_use_case/{model}_model_n_{i}.{model_extension}")
            else:
                ...
                self.evaluation_service.saveModel(f"./models/{model}/model/landing_use_case/{model}_model.{model_extension}")