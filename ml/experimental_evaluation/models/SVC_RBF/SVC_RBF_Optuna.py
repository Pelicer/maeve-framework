...
# @TODO: Merge with Linear and have parameters?
...
class SVC_RBF:
    ...
    def objective(self, trial):
        # Suggest values for C and gamma
    ...
    def create_and_train_model(self):
        ...
        study.optimize(self.objective, n_trials=50)  # Adjust n_trials for more extensive search

        best_params = study.best_trial.params

        # Retrain model with best parameters to get full metrics
        self.model = SVC(kernel='rbf', probability=True, **best_params)
    ...
    def train_and_evaluate(self):
        ...
        if not self.force_model_creation:
            if os.path.isfile('./models/SVC_RBF/model/SVC_RBF_model.pkl'):
                print("Force model creation is set to false. Loading latest saved model.")
                self.model = pypickle.load('./models/SVC_RBF/model/SVC_RBF_model.pkl')
            else:
